...
namespace App\SessionsHandler;

use Exception;
use SessionHandler;
use App\Database\Sessions;
...
class DBSessionsHandler  extends SessionHandler
{
     ...
     * @param  string $file for debug
     * @param  bool $debug 
     * @return void
     */
    public function __construct(int $session_duration = 3600, $authvaluename = null, string $file = null, bool $debug = false)
    {
    ...
    {
        if ($this->debug && !is_null($this->file)) {
            $base_uri = explode('?', $_SERVER['REQUEST_URI'], 2)[0];
            $hdl = fopen($this->file, 'a');
